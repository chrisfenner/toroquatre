...
func BenchmarkFind(b *testing.B) {
	for _, nodeLimit := range []int{4, 8, 16, 32, 64} {
		b.Run(fmt.Sprintf("nodeLimit=%d", nodeLimit), func(b *testing.B) {
			...
			for i := 0; i < 1000000; i++ {
				tree.Put(uint64(i), toroquatre.Vector{X: rand.Float64(), Y: rand.Float64()})
			}
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
				loc := tree.Get(uint64(rand.Int63n(1000000)))
				locTopLeft := toroquatre.Vector{X: loc.X - 0.01, Y: loc.Y - 0.01}
				if locTopLeft.X < 0.0 {
					locTopLeft.X += 1.0
				}
				if locTopLeft.Y < 0.0 {
				...
				locBottomRight := toroquatre.Vector{X: loc.X + 0.01, Y: loc.Y + 0.01}
